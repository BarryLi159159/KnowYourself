...
year = input("Enter the year of birth (e.g., 2023): ")
month = input("Enter the month of birth (e.g., 12): ")
day = input("Enter the day of birth (e.g., 25): ")
time = input("Enter the time of birth (24-hour format, e.g., 14): ")

is_female = input("Is the person female? (yes/no, default: no): ").strip().lower() == "yes"

# Options object with hard-coded defaults
options = argparse.Namespace(
    year=year,
    month=month,
    day=day,
    time=time,
    start=1900,  # Analysis year range start (hard-coded)
    end=2100,    # Analysis year range end (hard-coded)
    b=False,     # Always calculate Bazi internally
    g=True,      # Always use Gregorian calendar
    r=False,     # Always assume not a leap month
    n=is_female,
)
